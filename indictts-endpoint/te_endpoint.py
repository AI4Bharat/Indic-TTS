...
for lang in langs:
    ...
    models[lang]['synthesizer']  = Synthesizer(
        tts_checkpoint=f'saved_models/fastpitch/{lang}/male/best_model.pth',
        tts_config_path=f'saved_models/fastpitch/{lang}/male/config.json',
...
app = Flask(__name__)
socketio = SocketIO(app, async_mode='threading', cors_allowed_origins="*", path='tts_socket.io', async_handlers=True, pingTimeout=60000)
...
def infer(req_data):
    ...
    wavs = models[language]['synthesizer'].tts(text, speaker_name="", style_wav="")
    out = io.BytesIO()
